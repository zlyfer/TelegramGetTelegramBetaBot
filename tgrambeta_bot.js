...
bot.on("text", (message) => {
  ...
  bot.sendMessage(
    chatId,
    `This bot only serves a <strong>outdated version</strong> of Telgram Beta because the download source changed.\nYou will probably still be able to update via the app itself.\nIf you want to download the newest Beta version by yourself head over to: <a href="https://install.appcenter.ms/users/drklo-2kb-ghpo/apps/telegram-beta-2/distribution_groups/all-users-of-telegram-beta-2">App Center</a>\nSorry for the inconvenience, I will try to re-add the automatic download when I have time. Feel free to checkout the <a href="https://github.com/zlyfer/TelegramGetTelegramBetaBot/tree/nodejs">GitHub Repo</a> to contibute.`,
    { parse_mode: "HTML" }
  );
